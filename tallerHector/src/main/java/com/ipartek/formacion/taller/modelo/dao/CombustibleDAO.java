...
public class CombustibleDAO implements IDAO<Combustible> {
	...
	private static final String SQL_GET_BY_VEHICULO_ID = "SELECT c.id, c.nombre FROM combustible c, vehiculo v WHERE v.id_combustible = c.id AND v.id = ?;";
...
//	public Combustible getByVehiculoId(int id) {
//		Combustible c = null;
//		try (Connection conn = ConnectionManager.getConnection();
//				PreparedStatement pst = conn.prepareStatement(SQL_GET_BY_VEHICULO_ID);) {
//
//			pst.setInt(1, id);
//			try (ResultSet rs = pst.executeQuery()) {
//				while (rs.next()) {
