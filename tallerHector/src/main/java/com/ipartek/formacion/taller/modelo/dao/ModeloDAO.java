...
public class ModeloDAO implements IDAO<Modelo> {
	...
	private static final String SQL_GET_BY_MODELO_ID = "SELECT v.id, m.nombre FROM modelo m, vehiculo v WHERE v.id_modelo = m.id AND v.id = ?;";
...
//	public Modelo getByVehiculoId(int id) {
//		Modelo m = null;
//		try (Connection conn = ConnectionManager.getConnection();
//				PreparedStatement pst = conn.prepareStatement(SQL_GET_BY_MODELO_ID);) {
//
//			pst.setInt(1, id);
